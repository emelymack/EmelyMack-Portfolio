    ...
    <div className={style.formContainer}>
      <Form className={style.form} ref={form} onSubmit={sendEmail}>
        <Form.Group className="mb-3" controlId="from_name">
          ...
          <Form.Control type="text" placeholder={(lang === 'eng') ? 'Full name / Company' : 'Nombre completo / Empresa'} name="from_name" onChange={(e:React.ChangeEvent<HTMLInputElement>)=> handleInputChange(e)} className={`${style.input}`}/>
        </Form.Group>
        <Form.Group className="mb-3" controlId="from_email">
          <Form.Label className={style.label}>Email</Form.Label>
          <Form.Control type="email" placeholder="Email" name="from_email" onChange={(e:React.ChangeEvent<HTMLInputElement>)=> handleInputChange(e)} className={`${style.input}`} />
        </Form.Group>
        <Form.Group className="mb-3" controlId="message">
          <Form.Label className={style.label}>{(lang === 'eng') ? 'Message' : 'Mensaje'}</Form.Label>
          <Form.Control as="textarea" aria-label="Message" name="message" placeholder={(lang === 'eng') ? 'Your proposal, feedback, comments... everything is appreciated :)' : 'Tu propuesta, feedback, comentarios... todo se aprecia :)'} rows={5} onChange={(e:React.ChangeEvent<HTMLInputElement>)=> handleInputChange(e)} className={`${style.input} px-3 py-2`}/>
        </Form.Group>
